 ...
 * The LR parser header (`features.tab.h`) is a dependency for the transpiler.
 * 
*/
...
int main(int argc, char **argv) {
    ...
    /** Args validation */
    if (argc < 4) {
        fprintf(stderr, "\nERROR|Usage: features INFILE LOGFILE OUTFILE\n");
        exit(1);
    }
   ...
   /** Run the parser */
    if (!feat_transpile(in_file, log_file, out_file)) {
        status = 0;
